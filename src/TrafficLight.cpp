...
void TrafficLight::cycleThroughPhases()
{
    ...
    while (true)
    {
      ...
      if (time_since_last_update >= cycle_duration)
      {
        ...
        auto msg1 = _currentPhase; 
        auto sentFuture = std::async(std::launch::async, &MessageQueue<TrafficLightPhase>::send, &_msgqueue, std::move(msg1));
        sentFuture.wait();
